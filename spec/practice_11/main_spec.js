...
describe("Person", () => {
    ...
    describe("Teacher", () => {
        ...
        describe("#isTeaching", () => {
            it("should judge whether a student is teached by the teacher", () => {
                const teacher = new Teacher(1, "Tom", 21, klasses);
                const student = new Student(1, "jerry", 10, new Class(2));
                const isTeach = teacher.isTeaching(student);
                expect(isTeach).to.equal(true);
            });
            it("the student is not in the Teacher's class", () => {
                const teacher = new Teacher(1, "Tom", 21, klasses);
                const student = new Student(1, "jerry", 10, new Class(4));
                const isTeach = teacher.isTeaching(student);
                expect(isTeach).to.equal(false);
            });                
        });
    });
});
